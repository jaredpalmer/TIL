# LEMP Wordpress on Ubuntu 14 on Digitial Ocean

- Make a new droplet (with your SSH key)
- ssh into your new droplet
```bash
ssh root@my-droplets-ip
```
 - Pull the latest updates
```bash
apt-get update
```
 - Run the `do_user_script` for LEMP Wordpress. When prompted to enter a `MySQL` password, **just press the `enter` key**. Same with any other prompts.
```bash
#!/bin/sh
#
